                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div v-for="product in products" :key="product.id"
                                ...
                                <div class="p-4">
                                    ...
                                    <p class="text-gray-500 mb-2 line-clamp-2">{{ product.descripcion }}</p>
...
export default defineComponent({
    ...
    methods: {
        ...
        async loadUserProducts(userId: string) {
            try {
                // Here we would ideally have an API endpoint to get products by user ID
                // For now, we'll use the existing methods and filter client-side
                const allProducts = await this.productoComposable.getProductos();
                if (Array.isArray(allProducts)) {
                    this.products = allProducts.filter((product: Producto) => product.idUsuario === userId && product.activo);
                } else {
                    this.products = [];
                }
            } catch (error) {
